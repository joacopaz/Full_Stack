...
const determineRoundWinner = (user, ai) => {
    if ((user === 'rock' || user === 'paper' || user === 'scissors') && (ai === 'rock' || ai === 'paper' || ai === 'scissors')) {
        roundNumber++;
        if (user === ai) {
            return `You chose ${user} and the AI chose ${ai}. <br><br> It's a tie! `;
        } else if (user === 'rock' && ai === 'scissors' || user === 'paper' && ai === 'rock' || user === 'scissors' && ai === 'paper') {
            userScore++;
            return `You chose ${user} and the AI chose ${ai}. <br><br>Congratulations! You have won this round.`
        } else {
            aiScore++;
            return `You chose ${user} and the AI chose ${ai}. <br><br>Bummer, you have lost this round.`
        }
    } else {
        return `This round is invalid, a choice was incorrect. You chose ${user} and the ai chose ${ai}. Please replay the round.`
    }
}
...
const submit = () => {
    ...
    if (!userChoice) {
        document.getElementById('played').innerHTML = (`<div> Please make a choice. </div>`)
    ...
    if (roundsToPlay > roundNumber) {
        document.getElementById(
            "enquiry"
        ).innerText = `What do you chose for round ${roundNumber + 1}?`;
    } else {
        gameFinished = true;
    }
    if (roundNumber === 1) {
